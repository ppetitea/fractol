...
/*   Updated: 2019/02/12 17:14:22 by ppetitea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		manage_callback(void *param)
{
	...
	if (p->press)
	{
		...
		p->fp[p->fn].iterations *= p->key == KEY_PAD_ADD ? 1.5 : 1;
		p->fp[p->fn].iterations *= p->key == KEY_PAD_SUB
			&& p->fp[p->fn].iterations > 2 ? 0.75 : 1;
	}
	draw(p);
	loop(p);
	return (0);
